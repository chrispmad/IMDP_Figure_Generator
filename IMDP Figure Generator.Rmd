...
fig3_data = dat %>% 
  ...
  #pivot_longer(cols = c("NumberInsp","NumberHighRiskInsp",""))
...
p3 = fig3_data %>% 
  ggplot() +
  geom_col(aes(x=StationLabel, y=EncounterFreq), fill = my.grey, width = 0.40) +
  geom_point(aes(x = StationLabel, y = EncounterFreq/2, size = col.width),alpha = 0.5) +
  ...
  scale_y_continuous(name = "Encounter Frequency",
                     sec.axis = sec_axis(~., name = "% High Risk",
                                         breaks = seq(0,8,2))) +
  theme_classic() +
  scale_size_continuous(breaks = c(1,2,4),
                        labels = c("10 - 200",
                                   "200 - 1k",
                                   "1k- 10k")) +
  labs(x = "", y = "Encounter Frequency", size = "Total Inspections") +
...
p13 = dat %>% 
  filter(!Station %in% c(rovers_to_drop,"Osoyoos")) %>% 
  group_by(Station,Shift_ID) %>% 
